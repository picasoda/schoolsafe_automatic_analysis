...
✅ **지능형 판정** - 학교급별 기준값 적용  
✅ **보고서 생성** - 엑셀 자동 출력 (`{학교명}_분석결과.xlsx`)  
...
| **2** | `data_refining.py` | HTML 파싱 & 정제 | 학교별 HTML 응답 | 정제된 데이터 |
...
| 증상 | 원인 | 해결방법 |
|------|------|---------|
| `로그인 필요` 메시지 | 세션 만료 | 다시 실행하면 재로그인 |
| 테이블 찾을 수 없음 | HTML 구조 변경 | `HTML_SELECTORS` 업데이트 |
| 엑셀 파일 안 생성 | 권한 부족 | 폴더 쓰기 권한 확인 |
| Selenium 에러 | Chrome 버전 불일치 | ChromeDriver 버전 맞추기 |