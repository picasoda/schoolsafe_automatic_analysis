...
def parse_and_classify(html_content, code):
    ...
    try:
        ...
        min_education_hours = cfg["안전교육_총시간_기준"]
        ...
        min_types = cfg["재난대비훈련_종류"]
        min_total = cfg["재난대비훈련_총합"]
        
        if drill_types_cnt < min_types or total_drill < min_total:
            reasons.append(f"훈련(종류{drill_types_cnt}/합계{total_drill})")
            is_fail = True

        # 교육활동 참여자 수 추출 (팝업 구조 기준: staff 테이블의 10번째 칸)
        if staff_tbl and len(s_tds) >= 10:
            participant_count = _i(s_tds[9].text)

        # 이수율이 기준보다 낮으면 이유로 추가 (의심 판정 기준은 참여자 수로 변경)
        if staff_rate < cfg["이수율_기준"]:
            reasons.append(f"이수율({int(staff_rate*100)}%)")

        # '의심' 판정: 교육활동 참여자수가 config 기준 이상인 경우
        suspect_threshold = cfg["의심_참여자_기준"]
        if participant_count >= suspect_threshold:
            reasons.append(f"참여자수({participant_count}명)")
            is_suspect = True
        ...
        return {
            ...
            "미달내역": ", ".join(reasons),
            "교육활동참여자수": participant_count,
            "총시간": f"{education_hours_total}{KEYWORDS['data_display']['duration_short']}",
            "이수율": f"{int(staff_rate*100)}%",
            "훈련": f"종류{drill_types_cnt}/합계{total_drill}{KEYWORDS['data_display']['count_short']}"
        }
