...
def run_school_crawler():
    ...
    with open(config.SESSION_FILE_PATH, "w", encoding="utf-8") as f:
        json.dump(session_bundle, f, ensure_ascii=False, indent=4)
    print(f"✅ 세션 정보 저장 완료: {config.SESSION_FILE_PATH}")
    ...
    if response.status_code == 200:
        ...
        # 결과 저장
        if unique_codes:
            config.PATHS['school_list_json'].parent.mkdir(parents=True, exist_ok=True)
            with open(config.PATHS['school_list_json'], "w", encoding="utf-8") as f:
                json.dump(unique_codes, f, ensure_ascii=False, indent=4)
            print(f"🎉 성공! {len(unique_codes)}개의 학교 코드를 확보했습니다.")
            print(f"   저장 위치: {config.PATHS['school_list_json']}")
        else:
