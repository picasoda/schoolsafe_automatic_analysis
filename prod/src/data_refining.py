...
from config import PATHS, SESSION_FILE_PATH, API_CONFIG

# excel_manager는 루트에 있으므로 경로 추가
import sys
...
def run_data_refining(driver):
    ...
    session.headers.update({
        ...
        **API_CONFIG['headers']  # config에서 가져온 기본 헤더
    })
    
    # Origin과 Referer 동적 생성
    base_url = API_CONFIG['base_url']
    session.headers['Origin'] = base_url
    session.headers['Referer'] = base_url + API_CONFIG['endpoints']['list_page']

    # 5. 수집 루프
    for i, school_code in enumerate(target_list):
        # Payload 생성 (config의 키 이름 사용)
        payload_keys = API_CONFIG['payload_keys']
        payload = {
            payload_keys['school_code']: school_code,
            payload_keys['period']: period,
            payload_keys['page_type']: payload_keys['page_type_value']
        }

        try:
            response = session.post(
                API_CONFIG['base_url'] + API_CONFIG['endpoints']['detail_data'],
                data=payload,
                timeout=API_CONFIG['request_params']['timeout']
            )
        ...
        # 서버 부하 방지 딜레이
        time.sleep(API_CONFIG['request_params']['delay'])
